...
object paqueton {
  ...
  method pagosRealizados(unPago) {
    importePagado = (importePagado + unPago).min(self.precioTotal())
  }
  method estaPago() = importePagado == self.precioTotal() 
  method puedeSerEntregado(unMensajero) {
    return self.estaPago() && destinos.all({d => d.dejaPasar(unMensajero)}) // se pasa una condicion entre llaves.
  }
}